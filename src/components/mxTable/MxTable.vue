        ...
        <div class="overflow-x-auto p-3">
          <table class="table-auto w-full">
            ...
            <tbody class="text-sm divide-y divide-gray-100">
              <!-- record 1 -->
              <tr v-for="(article, index) in itemsList" :key="index">
                <td class="p-2">
                  <input
                    type="checkbox"
                    class="w-5 h-5"
                    value="id-1"
                    @click="toggleCheckbox(article.amount)"
                  />
                </td>
                <td class="p-2">
                ...
                <td class="p-2">
                  <div class="text-left">{{article.quantity}}</div>
                </td>
                <td class="p-2">
...
import { ref } from '@vue/reactivity';
	export default {
		...
		setup() {
			const amoutTtl = ref(0)
			const toggleCheckbox = (amount) => {
				amoutTtl.value = amoutTtl.value + amount
			}
